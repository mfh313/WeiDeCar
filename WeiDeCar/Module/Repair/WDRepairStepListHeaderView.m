...
-(void)setRepairStepModel:(WDRepairStepModel *)repairStep
{
    ...
    m_titleLabel.text = [NSString stringWithFormat:@"步骤%@",@(m_repairStep.repairOrder)];
...
{
    if ([self.m_delegate respondsToSelector:@selector(onClickFinishRepairStep:cellView:)]) {
        [self.m_delegate onClickFinishRepairStep:m_repairStep cellView:self];
    }
}
